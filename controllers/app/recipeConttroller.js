...
const latestRecipe = async (req, res, next) => {
  try {
    const recipes = await Recipe.find().sort({ createdAt: -1 }); // Sort by latest created
    if (recipes.length === 0) {
      return queryErrorRelatedResponse(req, res, 404, "No Racipes Found");
    }
...
module.exports = { latestRecipe, recipeById };
